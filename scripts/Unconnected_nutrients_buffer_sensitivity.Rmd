...
for(i in 1:length(N_P_buffers3)){
  ...
  for(z in 1:length(treatment)){
    ...
    df<-data.frame(buffer = buff, treatment_rate = treatment[z], N = n_treat, P = p_treat, leach_rate = leach[k], N_leach = n_leach, P_leach = p_leach)
...
test2<- test %>% 
  filter(buffer == 50 & treatment_rate == 0.2 & leach_rate == 0.1)
```
